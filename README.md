...
## 原理
1. 读取字库文件(ziku.txt)中的所有单字并放入list中
2. 生成指定长度的文字序列 (例: [0, 0, 0, 0])
3. 按照序列数字映射字库list元素对应的下标生成字符list (例: ['a', 'a', 'a', 'a'])
4. 最后将字符list转字符串并输出至文件 (out.txt)。

## 使用
将字库文件(ziku.txt)放在程序根目录下
仅需修改以下两行变量数值，即可控制生成的单句字数及单首句数。
``` 
length = 1  # 单句字数
poemSentence = 1  # 每首句数
```
生成结果(out.txt)将保存在程序根目录下
